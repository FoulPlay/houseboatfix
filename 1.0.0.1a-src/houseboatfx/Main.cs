...
//CHANGELOG:
//1.0.0.1a
//Reverted two fixes to see if babysitters or services or sims don't randomly get stuck under the houseboat in foundations or where boats are stored.
//Included Simler90's fix
...
namespace fp.houseboatfix
{
    ...
    public class Main
    {
        ...
        //Simler90's fix
        private static bool HouseboatRoutingShouldPreventSwimming(GameObject routingObject)
        {
            ...
            if (sim != null)
            {
                ...
                {
                    if (swimmingInPool.ContainerIsOcean)
                    {
                        return false;
                    }
                }
                ...
                {
                    Ocean.PondAndOceanRoutingPosture pondAndOceanRoutingPosture = sim.Posture as Ocean.PondAndOceanRoutingPosture;
                    if (pondAndOceanRoutingPosture != null && pondAndOceanRoutingPosture.WalkStyleToUse == Sim.WalkStyle.Wade)
                    {
                        return false;
                    }
                }
            }
            else if (routingObject is IBoat)
            {
                return false;
            }
            return true;
        ...
        //Part 1 of the fix. Allow Water Planning to stop the freezing when using Houseboats. (REVERTED)
        ...
        public static bool ExitHouseboatThroughBoat(GameObject routingObject, Route r, Lot houseboatLot, Houseboat houseboat, HouseboatJig houseboatJig)
        {
            ...
            r.SetOption(Route.RouteOption.EnableWaterPlanning, false); //Reverted from true to false to see if it fixes babysitters.
        ...
        public static bool EnterHouseboatThroughGangway(Vector3 startPos, GameObject routingObject, Route r, Lot houseboatLot, Houseboat houseboat, HouseboatJig houseboatJig, bool fromFixup)
        {
            ...
            if (Terrain.ArePointsConnectedByLand(startPos, position) && HouseboatRoutingShouldPreventSwimming(routingObject))
            {
        ...
        //Part 3 of the fix. Allow Water Planning to stop the freezing when using Houseboats. (REVERTED)
